...
BACKUP_DIR = os.environ.get('VM_MGR_BACKUP_DIR', '/var/lib/vm-backups')
# Setup logging
...
class LV:
    ...
    def get_pci_device_description(self, device_class):
        """Convert PCI device class code to human-readable description"""
        try:
            device_class = device_class.replace('0x', '')
            class_code = int(device_class, 16)
            base_class = (class_code >> 16) & 0xFF  # Get upper 8 bits
            subclass = (class_code >> 8) & 0xFF  # Get middle 8 bits
            
            if base_class == 0x01:  # Mass storage controller
                if subclass == 0x08:
                    return "Non-Volatile memory controller"
                else:
                    return "Storage controller"
            elif base_class == 0x03:  # Display controller
                if subclass == 0x00:
            ...
            elif base_class == 0x04:  # Multimedia controller
                if subclass == 0x03:
                    return "Audio device"
                else:
                    return "Multimedia controller"
            else:
                return f"PCI device (class {hex(class_code)})"
        except Exception:
            return "Unknown device"

    def get_pci_device_info(self, pci_addr):
        """Get PCI device information by address, regardless of whether it's free or attached"""
        base = '/sys/bus/pci/devices'
        full_addr = f"0000:{pci_addr}"
        try:
            device_path = os.path.join(base, full_addr)
            if not os.path.exists(device_path):
                return None
                
            with open(f'{device_path}/vendor') as f: vendor = f.read().strip()
            with open(f'{device_path}/device') as f: device = f.read().strip()
            with open(f'{device_path}/class') as f: device_class = f.read().strip()
            
            description = self.get_pci_device_description(device_class)
            device_name = self.get_pci_device_name(pci_addr)
    ...
    def get_pci_device_name(self, pci_addr):
        """Get detailed device name from lspci output"""
        try:
            # Run lspci to get detailed device information
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                # Remove the address prefix and get everything after the first colon
                if ':' in output:
                    parts = output.split(':', 2)  # Split into max 3 parts
                    if len(parts) >= 3:
                        # Get everything after "01:00.0 VGA compatible controller: "
                        device_name = parts[2].strip()
                        # Clean up revision info if present
                        if '(rev ' in device_name:
                            device_name = device_name.split('(rev ')[0].strip()
                        return device_name
                    elif len(parts) >= 2:
    ...
    def free_pci_devices(self):
        # Only return devices that are the sole member of their IOMMU group
        ...
        if os.path.isdir(group_root):
            for g in sorted(os.listdir(group_root)):
                ...
                try:
                    ...
                    # Read device class to get device type
                    with open(f'{base}/{dev}/class') as f: device_class=f.read().strip()
                    description = self.get_pci_device_description(device_class)
                    # Get detailed device name from lspci
                    device_name = self.get_pci_device_name(dev[5:])  # Use short address for lspci
                except Exception:
                    continue
                results.append({'full':dev,'addr':dev[5:],'vendor':vendor,'device':device,'group':g,'description':description,'name':device_name})
            return results
        # Fallback: old behavior if no IOMMU groups
        devs=[]; base='/sys/bus/pci/devices'
        if os.path.isdir(base):
            for e in os.listdir(base):
                if not e.startswith('0000:'): continue
                try:
                    with open(f'{base}/{e}/vendor') as f: vendor=f.read().strip()
                    with open(f'{base}/{e}/device') as f: device=f.read().strip()
                    with open(f'{base}/{e}/class') as f: device_class=f.read().strip()
                    description = self.get_pci_device_description(device_class)
                    device_name = self.get_pci_device_name(e[5:])
                except: continue
                devs.append({'full':e,'addr':e[5:],'vendor':vendor,'device':device,'group':'-','description':description,'name':device_name})
        return devs
            ...
            if snapshot_created:
                try:
                    if was_running:
                        ...
                        subprocess.check_call(['virsh', 'snapshot-delete', domain_name, snap_name])
                    else:
    ...
    def worker():
        ...
        try:
            ...
            if src.startswith('http://') or src.startswith('https://'):
                ...
                try:
                    ex=subprocess.check_output(['showmount','-e',host], text=True, timeout=8)
            ...
            try:
                ...
                def run_convert():
                    if convert_needed:
                        subprocess.check_call(['qemu-img','convert','-O','qcow2',local_path,dest_file])
                    else:
...
class Handler(BaseHTTPRequestHandler):
                ...
                if array_type == 'mdadm':
                    ...
                    try:
                        ...
                        if result.returncode == 0:
                            ...
                            if mount_point:
                                subprocess.run(['umount', mount_point], check=True, timeout=30)
                                # Remove from fstab
                                try:
                    ...
                    # Stop the array
                    subprocess.run(['mdadm', '--stop', device_path], check=True, timeout=30)
                    
                    # Zero superblocks on member devices
                    try:
                        result = subprocess.run(['mdadm', '--detail', device_path], 
                                              capture_output=True, text=True, timeout=10)
                    ...
                    try:
                        subprocess.run(['umount', array_name], check=True, timeout=30)
                        # Remove from fstab
                        with open('/etc/fstab', 'r') as f:
                            lines = f.readlines()
                        with open('/etc/fstab', 'w') as f:
        ...
        if path=='/remove_array' and form:
            ...
            try:
                ...
                if array_type == 'mdadm':
                    ...
                    try:
                        result = subprocess.run(['btrfs', 'filesystem', 'show'], capture_output=True, text=True, timeout=5)
                        if result.returncode == 0:
                            in_target_fs = False
                            for line in result.stdout.splitlines():
                    ...
                    for i,bus in enumerate(buses):
                        ...
                        if typ=='image':
                            ...
                            if not os.path.exists(dest): 
                                ...
                                try:
                                    subprocess.check_call(['cp', '--reflink=always', src_path, dest])
    ...
    def page_networks(self, lv: LV, form: Optional[dict] = None):
        ...
        if form:
            try:
                if 'create_network' in form:
                    name = form.get('net_name', [''])[0]
                    subnet = form.get('subnet', [''])[0]
                    dhcp = form.get('enable_dhcp', [''])[0] == '1'
                    
                    if name and subnet:
                        # Create simple isolated network
                        if dhcp:
                            network_xml = f"""<network>
                                <name>{name}</name>
                                <ip address='{subnet.split('/')[0]}' netmask='255.255.255.0'>
                                    <dhcp>
                                        <range start='{'.'.join(subnet.split('/')[0].split('.')[:-1])}.100' end='{'.'.join(subnet.split('/')[0].split('.')[:-1])}.200'/>
                                    </dhcp>
                                </ip>
                            </network>"""
                        else:
                            network_xml = f"""<network>
                                <name>{name}</name>
                                <ip address='{subnet}' netmask='255.255.255.0'/>
                            </network>"""
        ...
        for net in lv.list_networks():
            ...
            if ip_elem is not None:
                ...
                netmask = ip_elem.get('netmask', 'N/A')
                ip_range = f"{address}/{netmask}"
            
            # Create network control buttons outside f-string to avoid backslash issues
            net_name_escaped = net.name().replace("'", "\\'")
            if active:
                control_button = f'<button class="small secondary" onclick="controlNetwork(\'{net_name_escaped}\', \'stop\')">Stop</button>'
            else:
                control_button = f'<button class="small" onclick="controlNetwork(\'{net_name_escaped}\', \'start\')">Start</button>'
            
            network_rows.append(f"""
            <tr>
                <td>{html.escape(net.name())}</td>
                <td><span class="badge {'running' if active else 'shutoff'}">{'Active' if active else 'Inactive'}</span></td>
                <td>{ip_range}</td>
                <td>{'Yes' if autostart else 'No'}</td>
                <td>
                    {control_button}
                    <form method='post' class='inline'>
                        <input type='hidden' name='delete_network' value='1'>
                        <input type='hidden' name='net_name' value='{html.escape(net.name())}'>
                        <button class='small danger' onclick="return confirm('Delete network?')">Delete</button>
                    </form>
                </td>
            </tr>
            """)
        ...
        network_table = f"""
        <table>
            <thead><tr><th>Name</th><th>Status</th><th>IP Range</th><th>Autostart</th><th>Actions</th></tr></thead>
            <tbody>
                {''.join(network_rows) if network_rows else '<tr><td colspan="5"><em>No networks found</em></td></tr>'}
            </tbody>
        </table>
        """
        ...
        create_modal = """
        <div class='modal' id='modal_create_network' hidden>
            <div class='panel'>
                <div style='display:flex;justify-content:space-between;align-items:center'>
                    <h3>🌐 Create Virtual Network</h3>
                    <button class='close secondary' onclick="closeModal('modal_create_network')">×</button>
                </div>
                <form method='post'>
                    <input type='hidden' name='create_network' value='1'>
                    
                    <label>Network Name <input name='net_name' required placeholder='my-network'></label>
                    <label>Subnet (Gateway IP) <input name='subnet' required placeholder='192.168.100.1'></label>
                    <label><input type='checkbox' name='enable_dhcp' value='1' checked> Enable DHCP (100-200 range)</label>
                    
                    <div style='margin-top:16px'>
                        <input type='submit' class='button' value='Create Network'>
                        <button type='button' class='button secondary' onclick="closeModal('modal_create_network')">Cancel</button>
                    </div>
                </form>
            </div>
        </div>
    ...
    def page_dashboard(self, lv:LV):
        ...
        # Use the pre-calculated aggregate I/O values
        total_read_mbps = host_perf.get('total_rd_kbps', 0) / 1024
        total_write_mbps = host_perf.get('total_wr_kbps', 0) / 1024
        
        # Format with unit conversion (MB/s or GB/s)
        def format_rate(mbps):
            if mbps >= 1024:
                return f"{mbps/1024:.1f}GB/sec"
            else:
                return f"{mbps:.1f}MB/sec"
        
        # Always format the current values (even if zero)
        read_display = format_rate(total_read_mbps)
        write_display = format_rate(total_write_mbps)
        aggregate_io = f"Read: {read_display}<br>Write: {write_display}"
        ...
        <div class="stats-grid">
            ...
            <div class="stat-card">
                <div class="stat-value">{aggregate_io}</div>
                <div class="stat-label">Disk I/O (MB/s)</div>
            </div>
        </div>
        """
            ...
            setInterval(function() {
                fetch('/api/host-stats')
                    ...
                    .then(data => {
                        ...
                        // Update aggregate I/O (use pre-calculated totals)
                        if (data.total_rd_kbps !== undefined && data.total_wr_kbps !== undefined) {
                            const totalReadMbps = data.total_rd_kbps / 1024;
                            const totalWriteMbps = data.total_wr_kbps / 1024;
                            
                            // Always update with current values (including real zeros)
                            // Format with unit conversion (MB/sec or GB/sec)
                            function formatRate(mbps) {
                                if (mbps >= 1024) {
                                    return (mbps/1024).toFixed(1) + 'GB/sec';
                                } else {
                                    return mbps.toFixed(1) + 'MB/sec';
                                }
                            }
                            
                            const readDisplay = formatRate(totalReadMbps);
                            const writeDisplay = formatRate(totalWriteMbps);
                            const aggregateIo = 'Read: ' + readDisplay + '<br>Write: ' + writeDisplay;
                            document.querySelector('.stats-grid .stat-card:nth-child(3) .stat-value').innerHTML = aggregateIo;
                        }
    ...
    def get_disk_usage(self, disk_path):
        ...
        if not disk_path or not os.path.exists(disk_path):
            return 0, 0
        try:
            result = subprocess.run(['qemu-img', 'info', '--force-share', disk_path], 
                                  capture_output=True, text=True, timeout=10)
            if result.returncode == 0:
                    ...
                    if d.isActive():
                        ...
                        if new_v > cur_v:
                            try:
                                ...
                                msg += f"<div class='inline-note'>vCPUs increased to {new_v}.</div>"
                            except Exception as e:
                        ...
                        xml_cfg = d.XMLDesc(0)
                        xml_cfg = re.sub(r'<vcpu.*?>.*?</vcpu>', f'<vcpu>{new_v}</vcpu>', xml_cfg)
                        ...
                        if new_os_type == 'windows':
                            # Check QEMU support before adding full Hyper-V enlightenments
                            try:
                                ...
                                if qemu_binary:
                                    ...
                                    if result.returncode == 0 and 'hv-relaxed' in result.stdout:
                                        # Use safe Hyper-V features that should work
    ...
    <kvm>
        <hidden state='on'/>
                                        ...
                                        msg += f"<div class='inline-note'>Enabled safe Hyper-V enlightenments for Windows.</div>"
                                    else:
                                        # QEMU doesn't support Hyper-V, use basic features
                                        features_replacement = '''<features>
    <acpi/>
    <apic/>
</features>'''
                                        msg += f"<div class='inline-note'>QEMU doesn't support Hyper-V, using basic features for Windows.</div>"
                                else:
                                    # No QEMU binary found, use basic features
                                    features_replacement = '''<features>
    <acpi/>
    <apic/>
</features>'''
                                    msg += f"<div class='inline-note'>QEMU binary not found, using basic features for Windows.</div>"
                            except Exception as e:
                                # Error checking, use basic features for safety
                                features_replacement = '''<features>
    <acpi/>
    <apic/>
</features>'''
                                msg += f"<div class='inline-note'>Could not check QEMU capabilities, using basic features for Windows.</div>"
                        else:
    ...
    <apic/>
                        ...
                        if new_cpu_mode == 'host-passthrough':
                            cpu_replacement = "<cpu mode='host-passthrough'/>"
                        elif new_cpu_mode == 'custom':
                            cpu_replacement = f"<cpu mode='custom' match='exact' check='partial'><model fallback='allow'>{new_cpu_model}</model></cpu>"
                        elif new_cpu_mode.startswith('custom:'):
                            # Handle CPU model selected from QEMU dropdown - remove the 'custom:' prefix
                            clean_cpu_model = new_cpu_mode[7:]  # Remove 'custom:' prefix - use different variable name
                            cpu_replacement = f"<cpu mode='custom' match='exact' check='partial'><model fallback='allow'>{clean_cpu_model}</model></cpu>"
                        else:  # host-model (default)
                            cpu_replacement = "<cpu mode='host-model'/>"
                        
                        # Apply CPU changes to XML
                        if re.search(r'<cpu.*?</cpu>', xml_cfg, flags=re.DOTALL):
                            xml_cfg = re.sub(r'<cpu.*?</cpu>', cpu_replacement, xml_cfg, flags=re.DOTALL)
                        else:
                            # If no CPU node exists, add one after features
                            if '<features>' in xml_cfg:
                                xml_cfg = re.sub(r'(</features>)', f'\\1\n            {cpu_replacement}', xml_cfg)
                            else:
                                # Add after os section
                                xml_cfg = re.sub(r'(</os>)', f'\\1\n            {cpu_replacement}', xml_cfg)
                        
                        # Update boot order configuration
                        boot_devices = new_boot_order.split(',')
                        boot_xml = ""
                        for device in boot_devices:
                            device = device.strip()
                            if device == 'hd':
                        ...
                        # Remove existing boot elements and add new ones
                        xml_cfg = re.sub(r'<boot dev=[^>]*/?>', '', xml_cfg)
                        # Add boot order right before </os> tag
                        xml_cfg = re.sub(r'(</os>)', f'{boot_xml}\\1', xml_cfg)
                                ...
                                if new_bytes > cur_sz:
                                    try:
                                        if d.isActive() and hasattr(d,'blockResize'):
                                            ...
                                            subprocess.check_call(['qemu-img','resize',path,str(new_bytes)])
                ...
                if 'attach_pci' in form and not d.isActive(): 
                    ...
                    try:
                        ...
                        host = f"<hostdev mode='subsystem' type='pci' managed='yes'><source><address domain='0x{dd}' bus='0x{bb}' slot='0x{slot}' function='0x{func}'/></source></hostdev>"
                        d.attachDeviceFlags(host, libvirt.VIR_DOMAIN_AFFECT_CONFIG)
                        msg += "<div class='inline-note'>PCI device attached successfully.</div>"
                    except Exception as e:
                        msg += f"<div class='inline-note'>Failed to attach PCI device: {html.escape(str(e))}</div>"
                if 'detach_pci' in form and not d.isActive(): 
                    det = form.get('pci_detach_addr', [''])[0]
                    dom_xml = d.XMLDesc(0)
                    import xml.etree.ElementTree as ET
                    root = ET.fromstring(dom_xml)
                    for hd in root.findall('.//devices/hostdev'):
                        ...
                        rep = f"{int(a.get('domain','0'),16):04x}:{int(a.get('bus','0'),16):02x}:{int(a.get('slot','0'),16):02x}.{int(a.get('function','0'),16)}"
                        if rep == det:
                            d.detachDeviceFlags(ET.tostring(hd, encoding='unicode'), libvirt.VIR_DOMAIN_AFFECT_CONFIG)
                            msg += f"<div class='inline-note'>PCI {html.escape(det)} detached.</div>"
                            break
                if 'add_nic' in form: 
                    ntype = form.get('nic_type', ['bridge'])[0]
                    src = form.get('nic_source', ['virbr0'])[0]
                    model = form.get('nic_model', ['virtio'])[0]
                    nic_xml = (f"<interface type='bridge'><source bridge='{src}'/><model type='{model}'/></interface>" if ntype == 'bridge' else f"<interface type='network'><source network='{src}'/><model type='{model}'/></interface>")
                    ...
                    try:
                        ...
                        msg += f"<div class='inline-note'>Failed to add NIC: {html.escape(str(e))}</div>"
        ...
        # Extract current boot order from XML
        current_boot_order = "hd,cdrom,network"  # default
        try:
            boot_elements = root.findall('.//os/boot')
            if boot_elements:
                boot_devices_ordered = []
                for boot in boot_elements:
                    dev = boot.get('dev')
                    if dev:
                        boot_devices_ordered.append(dev)
                if boot_devices_ordered:
                    current_boot_order = ','.join(boot_devices_ordered)
        except Exception:
            pass
        disks = []
        cdroms = []
        for disk in root.findall('.//devices/disk'):
            tgt = disk.find('target'); src = disk.find('source')
            device_type = disk.get('device', 'disk')
            if tgt is not None:
                if device_type == 'cdrom':
                    cdroms.append((tgt.get('dev'), src.get('file') if src is not None else '', tgt.get('bus', 'ide')))
                else:
                    disks.append((tgt.get('dev'), src.get('file') if src is not None else '', tgt.get('bus', 'virtio')))
        pci_att = []
        for hd in root.findall('.//devices/hostdev'):
            a = hd.find('source/address')
            if a is not None:
        ...
        <form method='post' style='display: grid; grid-template-columns: repeat(auto-fit, minmax(150px, 1fr)); gap: 16px; align-items: end;'>
            ...
            <div class='form-group'>
                <label style='display: block; margin-bottom: 4px; font-size: 14px; color: var(--text-secondary);'>vCPUs</label>
                <input name='vcpus' type='number' value='{vcpu_display}' min='{vcpu_display}' style='width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 4px; background: var(--bg-secondary);'>
            </div>
            ...
            <div class='form-group'>
                <label style='display: block; margin-bottom: 4px; font-size: 14px; color: var(--text-secondary);'>Boot Order</label>
                <select name='boot_order' class='enh' style='width: 100%; padding: 8px; border: 1px solid var(--border); border-radius: 4px; background: var(--bg-secondary);'>
                    <option value='hd,cdrom,network'{'selected' if current_boot_order == 'hd,cdrom,network' else ''}>Hard Disk, CD/DVD, Network</option>
                    <option value='cdrom,hd,network'{'selected' if current_boot_order == 'cdrom,hd,network' else ''}>CD/DVD, Hard Disk, Network</option>
                    <option value='network,hd,cdrom'{'selected' if current_boot_order == 'network,hd,cdrom' else ''}>Network, Hard Disk, CD/DVD</option>
                    <option value='hd,network,cdrom'{'selected' if current_boot_order == 'hd,network,cdrom' else ''}>Hard Disk, Network, CD/DVD</option>
                </select>
            </div>
        ...
        for dev, path, bus in disks:
            ...
            disk_items.append(f"""
            <div class='card' style='margin: 8px 0; padding: 20px;'>
                <div style='display: grid; grid-template-columns: 1fr auto; gap: 24px; align-items: start;'>
                    <!-- Disk Information -->
                    <div style='display: flex; flex-direction: column; gap: 12px;'>
                        <div style='display: flex; align-items: center; gap: 12px; margin-bottom: 8px;'>
                            <span style='color: var(--text-secondary); font-size: 14px; background: var(--bg); padding: 4px 8px; border-radius: 4px; border: 1px solid var(--border);'>{html.escape(current_bus)} bus</span>
                            <span style='font-weight: 600; color: var(--accent); font-size: 16px;'>• {allocated_fmt} GB allocated</span>
                            <span style='font-weight: 600; color: var(--accent); font-size: 16px;'>• {used_fmt} GB used</span>
                        </div>
                        <div style='color: var(--text-secondary); font-size: 14px; display: flex; align-items: center; gap: 8px; margin-top: 8px;'>
                            <span style='color: var(--accent);'>💾</span>
                            <span style='word-break: break-all; flex: 1;'>Pool: {html.escape(current_pool)}</span>
                            <button type='button' class='button small secondary' onclick='openMigrationModal("{html.escape(dev)}", "{html.escape(current_pool)}")' style='padding: 4px 8px; font-size: 11px; white-space: nowrap;'>🔄 Migrate</button>
                        </div>
                        <!-- Progress Bar (always visible, hidden by default) -->
                        <div id='migration-progress-{html.escape(dev)}' style='margin-top: 8px; display: none;'>
                            <div style='background: var(--bg); border: 1px solid var(--border); border-radius: 4px; padding: 8px;'>
                                <div style='display: flex; align-items: center; gap: 8px; margin-bottom: 6px;'>
                                    <span style='color: var(--accent);'>🔄</span>
                                    <span id='migration-status-{html.escape(dev)}' style='font-size: 13px; font-weight: 500;'>Migrating disk...</span>
                                </div>
                                <div style='background: #f0f0f0; border-radius: 3px; overflow: hidden; height: 16px; margin-bottom: 4px;'>
                                    <div id='migration-bar-{html.escape(dev)}' style='background: var(--accent); height: 100%; width: 0%; transition: width 0.3s;'></div>
                                </div>
                                <div id='migration-percent-{html.escape(dev)}' style='font-size: 11px; color: var(--text-secondary);'>0%</div>
                            </div>
                        </div>
                    </div>
                    
                    <!-- Action Buttons in Grid -->
                    <div style='display: grid; grid-template-columns: auto auto; gap: 16px; justify-content: start; align-items: start;'>
                        <!-- Resize Disk -->
                        <form method='post' style='display: flex; flex-direction: column; gap: 6px;'>
                            <input type='hidden' name='resize_disk' value='1'>
                            <input type='hidden' name='disk_target' value='{html.escape(dev)}'>
                            <label style='font-size: 12px; color: var(--text-secondary); font-weight: 600;'>Resize Disk</label>
                            <div style='display: flex; gap: 6px; align-items: center;'>
                                <input name='new_size_gb' type='number' min='{allocated_fmt if allocated_fmt!='-' else 1}' placeholder='Size GB' style='width: 80px; padding: 6px 8px; font-size: 13px; border: 1px solid var(--border); border-radius: 4px; background: var(--bg);'>
                                <button type='submit' class='button small primary' style='padding: 6px 10px; font-size: 12px; white-space: nowrap;'>📈 Grow</button>
                            </div>
                        </form>
                        
                        <!-- Change Bus Type -->
                        <form method='post' style='display: flex; flex-direction: column; gap: 6px; {"opacity: 0.5;" if is_running else ""}'>
                            <input type='hidden' name='change_disk_bus' value='1'>
                            <input type='hidden' name='disk_target' value='{html.escape(dev)}'>
                            <label style='font-size: 12px; color: var(--text-secondary); font-weight: 600;'>Bus Type</label>
                            <div style='display: flex; gap: 6px; align-items: center;'>
                                <select name='new_bus' style='width: auto; min-width: 80px; padding: 6px 8px; font-size: 13px; border: 1px solid var(--border); border-radius: 4px; background: var(--bg);' {'disabled' if is_running else ''}>
                                ...
                                <button type='submit' class='button small secondary' {'disabled' if is_running else ''} style='padding: 6px 10px; font-size: 12px; white-space: nowrap;'>🔄 Change</button>
                            </div>
                        </form>
                        
                        <!-- Delete Disk -->
                        <form method='post' style='display: flex; flex-direction: column; gap: 6px; {"opacity: 0.5;" if is_running else ""}'>
                            <input type='hidden' name='detach_disk' value='1'>
                            <input type='hidden' name='target' value='{html.escape(dev)}'>
                            <label style='font-size: 12px; color: var(--text-secondary); font-weight: 600;'>Danger Zone</label>
                            <button type='submit' class='button small danger' {'disabled' if is_running else ''} onclick='return confirm("⚠️ Permanently delete disk {html.escape(dev)}?\\n\\nThis will remove the disk file and cannot be undone!")' style='padding: 6px 10px; font-size: 12px; white-space: nowrap;'>🗑️ Delete Disk</button>
                        </form>
                    </div>
                </div>
                {f'<div style="color: var(--text-secondary); font-size: 12px; text-align: center; margin-top: 16px; padding: 8px; background: var(--bg); border-radius: 4px; border: 1px solid var(--border);">⚠️ Stop VM to change bus type or delete disk</div>' if is_running else ''}
            </div>""")
        
        disk_list = f'<div class="section"><h3 style="margin: 16px 0 12px 0; display: flex; align-items: center; gap: 8px;"><span>💾</span> Virtual Disks</h3>{"".join(disk_items)}</div>' if disk_items else '<div class="section"><h3 style="margin: 16px 0 12px 0;">💾 Virtual Disks</h3><div class="card"><p style="color: var(--text-secondary); text-align: center; padding: 20px;">No virtual disks attached</p></div></div>'
        ...
        disk_list += migration_check_script
        ...
        disk_list = migration_modal + disk_list
        ...
        # PCI Device Passthrough - Filter for specific device types
        def pci_filter(dev):
            """Filter for Non-Volatile memory controller, VGA compatible controller, and Ethernet controller"""
            desc = dev.get('description', '').lower()
            return any(target in desc for target in [
                'non-volatile memory controller',
                'vga compatible controller', 
                'ethernet controller'
            ])
        
        # Get filtered PCI devices and currently attached ones
        free_pci_devices = [d for d in lv.free_pci_devices() if pci_filter(d)]
        attached_pci = []
        for hd_addr in pci_att:
            # Get device info for attached PCI devices using the new method
            dev_info = lv.get_pci_device_info(hd_addr[5:])  # Remove '0000:' prefix
            if dev_info:
                attached_pci.append(dev_info)
            else:
                # Fallback if device not found
                attached_pci.append({'addr': hd_addr[5:], 'description': 'Unknown', 'vendor': '', 'device': '', 'name': None})
        
        # Build PCI device passthrough UI
        pci_forms = ''
        
        # Show currently attached PCI devices
        if attached_pci:
            pci_forms += '''
            <div style="margin-bottom: 20px;">
            ...
            '''
            for i, dev in enumerate(attached_pci):
                addr_full = f"0000:{dev['addr']}"
                bg_color = "var(--bg-secondary)" if i % 2 == 0 else "var(--card)"
                
                # Use detailed device name if available
                if dev.get('name'):
                    device_title = f"{dev.get('description', 'Unknown')} - {dev['name']}"
                else:
                    device_title = dev.get('description', 'Unknown')
                
                pci_forms += f'''
                    <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 16px; background: {bg_color}; border-bottom: 1px solid var(--border);">
                        <div style="flex: 1;">
                            <div style="font-weight: 600; color: var(--fg);">{html.escape(device_title)}</div>
                            <div style="color: var(--muted); font-size: 13px;">
                                Address: {html.escape(addr_full)} | 
                                Vendor: {html.escape(dev.get('vendor', 'N/A'))} | 
                                Device: {html.escape(dev.get('device', 'N/A'))}
                            </div>
                        </div>
                        <form method="post" style="margin: 0;">
                            <input type="hidden" name="detach_pci" value="1">
                            <input type="hidden" name="pci_detach_addr" value="{html.escape(dev['addr'])}">
                            <button type="submit" class="button small danger" style="margin: 0;">🗑️ Detach</button>
                        </form>
                    </div>
                '''
            pci_forms += '</div></div>'
        ...
        if free_pci_devices and not d.isActive():
            pci_forms += '''
            <div style="margin-bottom: 20px;">
                <h4 style="margin: 0 0 12px 0; color: var(--fg); font-size: 16px;">➕ Attach PCI Device</h4>
                <div style="border: 1px solid var(--border); border-radius: 6px; padding: 16px; background: var(--card);">
            ...
            for dev in free_pci_devices:
                # Use detailed device name from lspci if available, otherwise fall back to vendor/device IDs
                if dev.get('name'):
                    device_display = f"{dev['description']} - {dev['name']}"
                else:
                    vendor_device = f"{dev.get('vendor', 'N/A')} {dev.get('device', 'N/A')}"
                    device_display = f"{dev['description']} - {vendor_device}"
                
                pci_forms += f'''
                                <option value="{html.escape(dev['addr'])}">{html.escape(device_display)} ({html.escape(dev['addr'])})</option>
                '''
            pci_forms += '''
                            </select>
                        </div>
            ...
            '''
        elif not free_pci_devices and not attached_pci:
            pci_forms += '''
            <div style="text-align: center; padding: 20px; color: var(--muted); font-size: 14px;">
                No compatible PCI devices found (NVMe, GPU, or Ethernet controllers)
            </div>
            '''
        elif d.isActive() and free_pci_devices:
            pci_forms += '''
            <div style="text-align: center; padding: 20px; background: var(--bg-secondary); border-radius: 6px; color: var(--muted); font-size: 14px;">
                ⏸️ Stop the VM to attach additional PCI devices
            </div>
            '''
        ...
        <div class='vm-content-grid' style='display: grid; grid-template-columns: 1fr; gap: 24px;'>
                    ...
                    <details>
                        ...
                        <div style='background: var(--bg); border: 1px solid var(--border); border-radius: 6px; padding: 12px; margin-top: 6px;'>
                            <form method='post' style='display: flex; flex-wrap: wrap; gap: 16px; align-items: end;'>
                                ...
                                <div class='form-group'>
                                    <label style='display: block; margin-bottom: 6px; font-size: 14px; font-weight: 500; color: var(--fg);'>Bus Type</label>
                                    <select name='bus' style='width: auto; min-width: 140px; padding: 10px; border: 1px solid var(--border); border-radius: 6px; background: var(--card); color: var(--fg);'>
                                ...
                                <div class='form-group'>
                                    <label style='display: block; margin-bottom: 6px; font-size: 14px; font-weight: 500; color: var(--fg);'>Template (optional)</label>
                                    <select name='template_disk' style='width: auto; min-width: 160px; padding: 10px; border: 1px solid var(--border); border-radius: 6px; background: var(--card); color: var(--fg);'>
                                ...
                                <div class='form-group'>
                                    <button type='submit' class='button primary' style='padding: 10px 16px; font-weight: 600;'>
                                        ➕ Add
                                    </button>
                                </div>
                            </form>
            ...
            <div class='network-media-section' style='display: grid; grid-template-columns: 1fr 1fr; gap: 24px;'>
                ...
                <div class='card'>
                    <h3 style='margin: 0 0 20px 0; display: flex; align-items: center; gap: 10px; color: var(--accent); font-size: 20px; border-bottom: 2px solid var(--border); padding-bottom: 12px;'>
                        <span>💿</span> CD/DVD Drives
                    </h3>
                    
                    <div style='margin-bottom: 20px;'>
                        <h4 style='margin: 0 0 12px 0; color: var(--fg); font-size: 16px;'>Optical Drives</h4>
                        {cdrom_list}
                    </div>
                    
                    <details>
                        <summary style='padding: 10px; background: var(--border); border-radius: 6px; cursor: pointer; font-weight: 600; margin-bottom: 12px;'>💿 Add CD/DVD Drive</summary>
                        <div style='background: var(--bg); border: 1px solid var(--border); border-radius: 6px; padding: 12px; margin-top: 8px;'>
                            <form method='post' style='display: grid; gap: 12px;'>
        ...
        # PCI Devices section (full width at bottom if present)
        if pci_forms:
            sections.append(f"""
            <div class='pci-section' style='margin-top: 24px;'>
                <div class='card'>
                    <h3 style='margin: 0 0 20px 0; display: flex; align-items: center; gap: 10px; color: var(--accent); font-size: 20px; border-bottom: 2px solid var(--border); padding-bottom: 12px;'>
                        <span>🔌</span> PCIe Device Passthrough
                    </h3>
                    {pci_forms}
                </div>
            </div>
    ...
    def page_create(self, lv:LV, form):
        ...
        if form and 'create_vm' in form:
            try:
                ...
                dom=f"""<domain type='kvm'><name>{name}</name><memory unit='MiB'>{mem}</memory><currentMemory unit='MiB'>{mem}</currentMemory><vcpu>{vcpus}</vcpu><os><type arch='x86_64' machine='pc'>hvm</type>{boot_xml}</os>{features_xml}<cpu mode='host-model'/><devices><emulator>/usr/libexec/qemu-kvm</emulator><disk type='file' device='disk'><driver name='qemu' type='qcow2'/><source file='{path}'/><target dev='vda' bus='virtio'/></disk><interface type='bridge'><source bridge='{bridge}'/><model type='{nic_model}'/></interface><graphics type='vnc' port='-1' listen='127.0.0.1'/><console type='pty'/></devices></domain>"""
    ...
    def page_storage(self, lv:LV, form):
                ...
                if 'create_btrfs_raid' in form:
                    ...
                    def create_btrfs_async():
                        try:
                            ...
                            subprocess.check_call(['mount','-o','noatime,ssd,compress=zstd',devs[0],mnt])
                            ...
                            try:
                                ...
                                if uuid:
                                    with open('/etc/fstab','a') as f:
                                        f.write(f"UUID={uuid} {mnt} btrfs noatime,ssd,compress=zstd 0 0\n")
                            except Exception:
                                pass
                ...
                if 'create_mdraid' in form:
                    ...
                    def create_raid_async():
                        try:
                            ...
                            cmd=['mdadm','--create',mdpath,'--level',level,f"--raid-devices={len(devs)}"]+devs
                            subprocess.check_call(cmd)
                            
                            PROGRESS[raid_pid]['pct'] = 40
                            PROGRESS[raid_pid]['msg'] = 'Creating XFS filesystem'
                            subprocess.check_call(['mkfs.xfs','-f',mdpath])
                            
                            PROGRESS[raid_pid]['pct'] = 70
                            PROGRESS[raid_pid]['msg'] = 'Mounting filesystem'
                            os.makedirs(mnt,exist_ok=True)
                            subprocess.check_call(['mount',mdpath,mnt])
                            
                            PROGRESS[raid_pid]['pct'] = 90
                            PROGRESS[raid_pid]['msg'] = 'Updating fstab'
                            try:
                                uuid=subprocess.check_output(['blkid','-s','UUID','-o','value',mdpath],text=True).strip()
                                if uuid:
                                    with open('/etc/fstab','a') as f:
                ...
                for line in result.stdout.splitlines():
                    if 'uuid:' in line.lower():
                        ...
                        if 'Label:' in line:
                            ...
                            if label_idx != -1:
                                current_label = line[label_idx + 6:].strip().strip("'\"")
                        else:
                            current_label = None
        ...
        if existing_raids:
            ...
            for raid in existing_raids:
                ...
                if raid['status'] == 'mounted':
                    ...
                    # Only show remove button for unmounted filesystems
                    if raid['type'] == 'mdadm':
        ...
        for s in md_status.get('devices',[]):
            md_rows+=(
                f"<tr id='md_{html.escape(s['name'])}'><td>{html.escape(s['name'])}</td><td style='min-width:160px'><div style='background:#222;border:1px solid #333;height:14px;position:relative;border-radius:4px;overflow:hidden'><div class='bar' style='background:var(--accent);height:100%;width:{s['percent']}%'></div></div></td><td class='state'>{html.escape(s.get('state',''))}</td><td class='eta'>{html.escape(s.get('eta',''))}</td><td class='speed'>{html.escape(s.get('speed',''))}</td><td style='overflow:visible;position:relative'><select name='speed' class='enh'><option value='pause'>pause</option><option value='normal' selected>normal</option><option value='fast'>fast</option></select></td><td><button class='small danger' onclick=\"if(confirm('Delete RAID array {html.escape(s['name'])}? This will destroy all data!')) {{ fetch('/delete_array', {{ method: 'POST', headers: {{'Content-Type': 'application/x-www-form-urlencoded'}}, body: 'type=mdadm&name={html.escape(s['name'])}' }}).then(() => location.reload()); }}\">Delete</button></td></tr>"
            )
        md_table=(
            "<h4>mdraid Sync</h4><table style='overflow:visible'><thead><tr><th>Array</th><th>Progress</th><th>Status</th><th>ETA</th><th>Speed</th><th>Mode</th><th>Actions</th></tr></thead><tbody style='overflow:visible'>"
            + md_rows
            + "</tbody></table>" if md_rows else ''
        )
        buttons=(
    ...
    def block_devices(self):
        ...
        try:
            ...
            out=subprocess.check_output(['lsblk','-J','-b','-o','NAME,SIZE,MODEL,SERIAL,TYPE,TRAN,MOUNTPOINT'], text=True)
    ...
    def page_btrfs(self, lv:LV, form):
        ...
        if form and 'create_subvol' in form:
            try:
                ...
                # create subvolume
                subprocess.check_call(['btrfs','subvolume','create', os.path.join(parent,sub)])
                os.makedirs(mnt, exist_ok=True)
                ...
                subprocess.check_call(['mount','-o',mount_opts,dev,mnt])
                if addfstab:
                    # attempt to get UUID
                    try:
                        out=subprocess.check_output(['blkid','-s','UUID','-o','value',dev], text=True).strip()
                        if out:
